...
ul.setAttribute("class", "pagination justify-content-center pagination-wrap d-flex");
...
function previous(j) {
  if (j != 0) {
    previousa.setAttribute("href", "javascript:page(" + j + ");previous(" +(j-1)+ ");");
  }
}
...
for (let i = 1; i <= 5; i++) {
  ...
  numli.setAttribute("class", "page-item");
  ul.append(numli);
  let numa = document.createElement("a");
  numa.setAttribute("class", "page-link");
  numa.setAttribute(
    "href",
...
function next(j) {
  if (j != 11) {
    nexta.setAttribute("href", "javascript:page(" + j + ");next(" +(j+1)+ ");");
  }
}
...
tr.setAttribute("class", "table-active text-light");
table.append(tr);
...
for (let i = 0; i < 20; i++) {
  ...
  tr1.setAttribute("class", " table-success text-dark");
  table.append(tr1);
  ...
  nexta.setAttribute("href","javascript:next(1);");
}

function page(j) {
  let r=20;
  for (let i = (j - 1) * r; i < (j - 1) * r + r; i++) {

    nexta.setAttribute("href","javascript:next("+ (j+1) +");");
    previousa.setAttribute("href","javascript:previous("+ (j-1) +");");
